/**
 * Скрипт загрузки событий за ноябрь 2025 (01.11.2025 - 23.11.2025)
 * Загружает данные для всех организаций и сотрудников
 *
 * Запуск: node load_november_events.js
 */
...
const REPORT_FILE = path.join(__dirname, 'load_november_report.txt');
...
function generateReport() {
  ...
  let report = '';
  report += '='.repeat(80) + '\n';
  report += '          ОТЧЕТ О ЗАГРУЗКЕ СОБЫТИЙ ЗА НОЯБРЬ 2025\n';
  report += '='.repeat(80) + '\n\n';
  ...
  report += '─'.repeat(80) + '\n';
  report += `Всего организаций:              ${executionStats.organizations.length}\n`;
  ...
  report += '─'.repeat(80) + '\n';
  report += 'ДЕТАЛИЗАЦИЯ ПО ОРГАНИЗАЦИЯМ\n';
  report += '─'.repeat(80) + '\n\n';
  ...
  if (executionStats.failedOrganizations === 0) {
    report += '✓ ВСЕ ОРГАНИЗАЦИИ УСПЕШНО ОБРАБОТАНЫ!\n';
  } else if (executionStats.successOrganizations > 0) {
    report += `⚠ ЧАСТИЧНЫЙ УСПЕХ: ${executionStats.successOrganizations} из ${executionStats.organizations.length} организаций обработаны\n`;
  } else {
...
async function loadOrganizationData(organization) {
  ...
  const orgStats = {
    bin: organization.object_bin,
    name: organization.object_company,
    status: 'error',
    employeesCount: 0,
    eventsLoaded: 0,
    recordsProcessed: 0,
  ...
  console.log('\n' + '─'.repeat(80));
  console.log(`📦 Загрузка: ${organization.object_company} (${organization.object_bin})`);
...
async function main() {
  console.log('\n' + '='.repeat(80));
  console.log('🚀 ЗАПУСК ЗАГРУЗКИ СОБЫТИЙ ЗА НОЯБРЬ 2025');
  console.log('='.repeat(80));
  ...
  try {
    ...
    for (let i = 0; i < organizations.length; i++) {
      ...
      console.log(`\n[${i + 1}/${organizations.length}] Обработка организации...`);
    ...
    console.log(`📊 Успешных организаций: ${executionStats.successOrganizations}/${executionStats.organizations.length}`);
