...
**Входные данные**:
- `reviews` - отзывы клиентов (до 50 штук с полным текстом)
...
- ✅ Детальный анализ текста отзывов (до 300 символов)
...
### ✅ Новые возможности (2025-07-15):

**📝 Просмотр промптов агентов:**
- Каждый блок результата агента содержит две вкладки:
  - **"📊 Результат анализа"** - результат работы агента
  - **"📝 Отправленный промпт"** - детали запроса к AI
- Показывает полный текст промпта, провайдера, время выполнения, количество токенов
- Функция копирования промпта в буфер обмена
- Поддержка системного и основного промптов

**🔍 Улучшенная обработка отзывов:**
- Обрабатываются все запрошенные отзывы (не ограничено 10)
- Полный текст отзывов (до 300 символов вместо 50)
- Правильная обработка поля `text` в отзывах
...
## 🆕 Последние обновления (2025-07-15)

### 📝 Система просмотра промптов
- **Новый API**: `GET /api/admin/ai-recommendations/prompts/:analysisId`
- **Интерфейс**: Вкладки "Результат анализа" и "Отправленный промпт" для каждого агента
- **Детали промпта**: Провайдер, время выполнения, количество токенов, статус
- **Копирование**: Функция копирования промпта в буфер обмена
- **Системные промпты**: Отображение как основных, так и системных промптов

### 🔍 Исправление обработки отзывов
- **Проблема**: Ограничение до 10 отзывов вместо запрошенных 50
- **Решение**: Убрано принудительное ограничение `slice(0, 10)`
- **Проблема**: Пустые тексты отзывов из-за неправильного поля `comment`
- **Решение**: Поддержка полей `text` и `comment` с приоритетом `text`
- **Проблема**: Обрезка текста до 50 символов
- **Решение**: Увеличен лимит до 300 символов для сохранения контекста

### 🧪 Тестирование
- **Новый тест**: `test_reviews_fix.js` - полное тестирование обработки отзывов
- **Валидация**: Проверка всего пути от MCP API до промпта агента
- **Результат**: ✅ Все 50 отзывов обрабатываются с полным текстом